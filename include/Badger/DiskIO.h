...
void writeFileToDisk(const std::shared_ptr<class FileInMemory>& file, const fs::path& destPath);
void writeFileToMultipleDestinations(const std::shared_ptr<class FileInMemory>& file, const std::vector<fs::path>& destPaths);

void writeDirectoryToDisk(const std::shared_ptr<class DirectoryInMemory>& directory, const fs::path& destPath);
